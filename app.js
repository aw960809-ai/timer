(() => {
  ...
  // ===== 歷史（容錯）=====
  const HISTORY_KEY = "extreme_control_training_history_pwa_v3";
  ...
  // ===== 調戲（不堆疊 + 避免重複）=====
  function getTeaseDict(){
    return teaseMode === "adult" ? teaseLinesAdult : teaseLinesClean;
  }
  ...
  function renderHistory(){
    ...
    for (const it of arr){
      ...
      li.innerHTML = `
        <div><strong>${it.characterName}</strong> <span class="muted">${it.endedAt}</span></div>
